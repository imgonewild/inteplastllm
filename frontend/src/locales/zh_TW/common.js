...
const TRANSLATIONS = {
  onboarding: {
    ...
    llm: {
      title: "LLM 偏好",
      description:
        "InteplastLLM 可以與多家 LLM 提供商合作。這將是處理聊天的服務。",
    },
    userSetup: {
      title: "使用者設定",
  ...
  welcomeMessage: {
    ...
    part2:
      "InteplastLLM 是將 OpenAI、GPT-4、LangChain、PineconeDB、ChromaDB 和其他強大 AI 產品整合在一起的最簡單方法，它能透過簡潔的套件，輕鬆地將您的生產力提高 100 倍。",
    part3:
      "InteplastLLM 可以完全在您的本機電腦上執行，而且使用極少的資源，您甚至不會注意到它的存在！不需要 GPU。同時也支援雲端和企業內部部署。\nAI 工具生態系統日新月異，AnythingLLM 讓使用變得更加簡單。",
  ...
  api: {
    title: "API 金鑰",
    description:
      "API 金鑰允許持有者以程式化方式存取和管理此 InteplastLLM 系統。",
    link: "閱讀 API 文件",
  ...
  llm: {
    title: "LLM 偏好設定",
    description:
      "這些是您偏好的 LLM 對話與嵌入提供者的憑證和設定。確保這些金鑰是最新且正確的，否則 InteplastLLM 將無法正常運作。",
    provider: "LLM 提供者",
    providers: {
      azure_openai: {
  ...
  transcription: {
    ...
    provider: "語音轉錄提供者",
    "warn-start":
      "在記憶體或處理器資源有限的電腦上使用本機 Whisper 模型，處理媒體檔案時可能會造成 InteplastLLM 停頓。",
  ...
  embedding: {
    ...
    "desc-end":
      "嵌入是將文字轉換成向量的過程。這些憑證是用於將您的檔案和提示詞轉換成 InteplastLLM 可以處理的格式。",
    provider: {
      title: "向量嵌入提供者",
      description: "使用 InteplastLLM 的原生嵌入引擎時，不需要任何設定。",
    },
  },
  text: {
    title: "文字分割與區塊化偏好設定",
  ...
  vector: {
    title: "向量資料庫",
    description:
      "這些是您的 InteplastLLM 系統運作方式的憑證和設定。確保這些金鑰是最新且正確的，這點非常重要。",
    provider: {
      title: "向量資料庫提供者",
  ...
  multi: {
    ...
    password: {
      title: "密碼保護",
      description:
        "使用密碼保護您的 InteplastLLM 系統。如果您忘記這個密碼，將沒有復原方法，因此請務必儲存此密碼。",
    },
    instance: {
      title: "系統密碼保護",
      description:
  ...
  privacy: {
    title: "隱私與資料處理",
    description:
      "這是您針對已連線的第三方供應商和 InteplastLLM 如何處理您的資料的設定。",
    llm: "LLM 選擇",
    embedding: "向量嵌入偏好設定",
    vector: "向量資料庫",
    anonymous: "已啟用匿名統計資訊",
  ...
  connectors: {
    ...
    upload: {
      ...
      "fetch-website": "擷取網站",
      "privacy-notice":
        "這些檔案將上傳到此 InteplastLLM 實例中的文件處理器。這些檔案不會發送或共享給第三方。",
    },
    pinning: {
      what_pinning: "什麼是文件固定？",
      pin_explained_block1:
        "當您在 InteplastLLM 中<b>固定</b>一個文件時，我們會將該文件的所有內容注入到您的提示窗口中，讓您的 LLM 完全理解。",
      pin_explained_block2:
        "這對於<b>大範圍模型</b>或對知識庫至關重要的小型文件效果最佳。",
      pin_explained_block3:
        "如果您沒有從 InteplastLLM 預設獲得理想的答案，那麼固定是一個輕鬆獲得更高質量答案的方法。",
      accept: "好的，明白了",
    },
    watching: {
      what_watching: "觀看文件有何作用？",
      watch_explained_block1:
        "當您在 InteplastLLM 中<b>觀看</b>一個文件時，我們會<i>自動</i>定期同步該文件的內容，並在每個管理該文件的工作區中自動更新內容。",
  ...
  customization: {
    interface: {
      title: "介面偏好設定",
      description: "設定你在 InteplastLLM 的使用介面偏好。",
    },
    branding: {
      title: "品牌與白標設定",
      description: "使用自訂品牌設計將 InteplastLLM 白標化。",
    },
    chat: {
      title: "聊天",
      description: "設定你在 InteplastLLM 的聊天偏好。",
      auto_submit: {
        title: "語音輸入自動送出",
    ...
    items: {
      ...
      "display-language": {
        title: "顯示語言",
        description: "選擇 InteplastLLM 使用者介面的顯示語言（如有提供翻譯）。",
      },
      logo: {
        title: "品牌標誌",
