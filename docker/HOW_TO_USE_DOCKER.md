...
Use the Dockerized version of InteplastLLM for a much faster and complete startup of InteplastLLM.

### Minimum Requirements

> [!TIP]
> Running InteplastLLM on AWS/GCP/Azure?
...
\*InteplastLLM by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*InteplastLLM by default embeds text on instance privately [Learn More](../server/storage/models/README.md)
...
> the docker container using InteplastLLM as `localhost:xxxx` will not resolve for the host system.
>
...
<table>
<tr>
<th colspan="2">Mount the storage locally and run InteplastLLM in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
The integrations below are templates or tooling built by the community to make running the docker experience of InteplastLLM easier.
...
After setting that up install the InteplastLLM docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in InteplastLLM.
